 ...
 const PreLeaveView = ({ currentMonth, employees, daysInMonth, currentUser, schedule, setSchedule, preLeaveData, setPreLeaveData, savePreLeaveMonth, saveMetaPreLeave, saveScheduleMonth, saveMonthDoc, runLotteryTransaction, resetMonthDraw, restorePreUnlockBackup }) => {
  ...
  useEffect(() => {
    ...
    const handleAutoLotteryCheck = async () => {
      try {
        ...
        //    且「沒有被管理員手動暫停自動抽籤」（例如解鎖後正在等同仁重新填寫預假申請階段）
        const isAutoSuspended = (preLeaveData.autoSuspendedMonths || []).includes(currentMonth);
        if (!isMonthDrawn && now >= drawTimeTimestamp && !isAutoSuspended) {
  ...
  const handleExportMonthJSON = () => {
    const monthBackup = {
      ...
      lotteryResults: preLeaveData.lotteryResults?.[currentMonth] || {}
    };
    ...
    const unsubMonths = onSnapshot(monthlyColRef, (snap) => {
      ...
      setPreLeaveData(prevPreLeave => {
        ...
        snap.docChanges().forEach(change => {
          ...
          // 💡 抽籤結果快照，依各月份自己的文件為準
          nextLotteryResults[m] = data.lotteryResults || {};
