...
const SwapRequestModal = ({ isOpen, onClose, onConfirm, data }) => {
  ...
  // 取得參與者列表，若無則回退到單人顯示 (確保相容性)
  const participants = data.participants || [
    { name: data.creatorName, shift: data.creatorShift, role: 'creator' },
    { name: data.targetName, shift: data.targetShift, role: 'target' }
  ];
    ...
    <div className="fixed inset-0 z-[600] flex items-center justify-center bg-black bg-opacity-60 p-4 font-sans backdrop-blur-md">
      <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden animate-in slide-in-from-bottom duration-300">
        ...
        <div className="bg-gradient-to-r from-cyan-600 to-blue-700 p-6 text-white text-center">
          ...
          <h3 className="text-xl font-black">換班確認</h3>
          ...
          <p className="text-blue-100 text-[10px] mt-1">
            {participants.length > 2 ? `多人連鎖換班 (${participants.length}人)` : '單日班別對調'}
          </p>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-3">
            {participants.map((p, idx) => (
              <React.Fragment key={idx}>
                <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-2xl border border-gray-100 shadow-sm">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs ${idx === 0 ? 'bg-blue-600 text-white' : 'bg-cyan-100 text-cyan-700'}`}>
                    {idx + 1}
                  </div>
                  <div className="flex-grow">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="bg-white px-2 py-0.5 rounded border border-gray-200 font-mono text-[11px] font-black text-blue-600 shadow-sm">
                        {p.shift}
                      </span>
                    </div>
                    <div className="font-black text-gray-700 text-sm">{p.name}</div>
                  </div>
                </div>
                {/* 顯示下一個人的連接箭頭 */}
                {idx < participants.length - 1 && (
                  <div className="flex justify-center -my-2 relative z-10">
                    <div className="bg-white p-1 rounded-full border border-gray-100 shadow-sm">
                      <ChevronRight className="rotate-90 text-blue-400" size={14} />
                    </div>
                  </div>
                )}
              </React.Fragment>
          ...
          {isBundle && (
            <div className="text-[10px] text-gray-400 text-center italic bg-gray-50 p-2 rounded-xl">
              注意：系統將自動對調該整段週期之班別
            </div>
          )}
          
          <div className="bg-blue-50 p-3 rounded-xl border border-blue-100 text-[10px] text-blue-700 leading-tight">
            <ShieldCheck className="inline mr-1" size={12}/> 
            送出後，系統將<strong>同時通知</strong>所有參與同仁。需全員核定後，才會送交組長終審。
          </div>
        </div>
...
const RecordsView = ({ currentUser, swapRequests, onAction, schedule, currentMonth }) => {
  ...
  const StatusProgress = ({ req }) => {
    ...
    return (
      // 在 RecordsView 渲染申請單的地方
      <div className="mt-2 flex flex-wrap gap-2">
      {req.participants.map(p => (
        <div key={p.id} className={`flex items-center gap-1 px-2 py-1 rounded-full border text-[10px] font-black ${req.approvals?.[p.id] ? 'bg-green-50 text-green-600 border-green-200' : 'bg-gray-50 text-gray-400 border-gray-100'}`}>
        {req.approvals?.[p.id] ? <CheckCircle2 size={12}/> : <Circle size={12}/>}
        {p.name}
      </div>
   ))}
  </div>
...
const handleSwapApply = (targetEmp, dayInfo) => {
    ...
    if (!swapTarget) {
      ...
      } else if (type === 'P' && (dOfW === 6 || dOfW === 0 || (dOfW >= 1 && dOfW <= 4))) {
        isBundle = true;
      ...
      setSwapTarget({
        ...
        daysToSwap,
        participants: [
          { id: currentUser.id, name: currentUser.name, shift: myShift, role: 'creator' },
          { id: targetEmp.id, name: targetEmp.name, shift: targetShift, role: 'target' }
        ],
        approvals: { [currentUser.id]: true }
      });
    } else {
      // 多人換班邏輯
      if (swapTarget.isBundle || type) {
        alert("多人換班或特定整段班別（A/P班）不支援多方鏈條參與。");
        return;
      }
      ...
      setSwapTarget(prev => ({
        ...prev,
        participants: [...prev.participants, { id: targetEmp.id, name: targetEmp.name, shift: targetShift, role: 'target' }]
      }));
    }
  }; // 確保這裡只有一個 }; 且直接銜接下一個函數
...
const handleRecordAction = (req, action) => {
  ...
  const validateShifts = () => {
    const targetMonthKey = req.date.substring(0, 7);
    const pts = req.participants || [{ id: req.creatorId, name: req.creatorName, shift: req.creatorShift }, { id: req.targetId, name: req.targetName, shift: req.targetShift }];
    return pts.every(p => {
      const currentShift = (schedule[targetMonthKey]?.[p.name]?.[req.day] || "-").toString().trim();
      const originalShift = (p.shift || "-").toString().trim();
      const clean = (s) => s.replace(/#|\(國\)/g, '');
      return clean(currentShift) === clean(originalShift);
    });
  };
  ...
  if (action === 'Approve') {
    // 即時檢核班別是否已被動過
    if (!validateShifts()) {
      alert("檢核失敗：其中一位同仁的班別已被更動，此申請單已自動作廢。");
      handleRecordAction(req, 'Delete'); 
      return;
    }

    if (req.status === 'PendingTarget') {
      const nextApprovals = { ...(req.approvals || {}), [currentUser.id]: true };
      const pts = req.participants || [];
      const allApproved = pts.length > 0 ? pts.every(p => nextApprovals[p.id] === true) : true;
      
      const nextRequests = swapRequests.map(r => r.id === req.id ? { 
        ...r, 
        approvals: nextApprovals, 
        status: allApproved ? 'PendingAdmin' : 'PendingTarget' 
      } : r);
      ...
      // 多人環形對調
      if (pts.length > 1) {
        const firstPersonShift = ns[targetMonthKey][pts[0].name][req.day];
        for (let i = 0; i < pts.length; i++) {
          const currentName = pts[i].name;
          const nextName = pts[(i + 1) % pts.length].name;
          ns[targetMonthKey][currentName][req.day] = ns[targetMonthKey][nextName][req.day];
          if (i === pts.length - 1) ns[targetMonthKey][currentName][req.day] = firstPersonShift;
        }
      }

      // 解鎖所有人
      const participantIds = pts.map(p => p.id);
      const nextEmps = employees.map(e => {
        if (participantIds.includes(e.id)) {
          return { ...e, applyingDates: (e.applyingDates || []).filter(d => d !== req.date) };
        }
        return e;
      });

      const nextRequests = swapRequests.map(r => r.id === req.id ? { ...r, status: 'Approved' } : r);
      setSchedule(ns);
      setEmployees(nextEmps);
      setSwapRequests(nextRequests);
      saveData({ schedule: ns, employees: nextEmps, swapRequests: nextRequests });
    }
  } 
  else if (action === 'Reject' || action === 'Delete') {
    const pts = req.participants || [{ id: req.creatorId }, { id: req.targetId }];
    const participantIds = pts.map(p => p.id);

    const nextRequests = (action === 'Delete') 
      ? swapRequests.filter(r => r.id !== req.id)
      : swapRequests.map(r => r.id === req.id ? { ...r, status: 'Rejected', rejectBy: currentUser.name } : r);
    ...
    alert(action === 'Delete' ? "申請已撤回並解鎖。" : `已由 ${currentUser.name} 否決並解鎖相關日期。`);
  }
};
...
onConfirm={() => {
  ...
  // 核心優化：鎖定所有參與者的該日期
  const participantIds = swapTarget.participants.map(p => p.id);
  const nextEmps = employees.map(e => {
    if (participantIds.includes(e.id)) {
      const currentDates = e.applyingDates || [];
      return { ...e, applyingDates: Array.from(new Set([...currentDates, targetDateStr])) };
    }
    return e;
  });
  ...
  setEmployees(nextEmps);
  saveData({ swapRequests: [...swapRequests, newRequest], employees: nextEmps });

  // 更新當前使用者狀態 (如果是參與者之一)
  if (participantIds.includes(currentUser.id)) {
    setCurrentUser(prev => ({
      ...prev,
      applyingDates: Array.from(new Set([...(prev.applyingDates || []), targetDateStr]))
    }));
  }
  setSwapTarget(null);
}}
